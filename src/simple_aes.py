...
def aes_shift_rows(byte_matrix):
    ...
    # Shift each row by the appropriate offset
    for r in range(4):
        for c in range(4):
...
def aes_round_trans():
    ...
    plain_text = [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10]
    ...
    key_matrix = array_to_matrix(round_key)
    print("key-matrix  :\n")
    ...
    for i in range(0,4):
        for j in range(0,4):
            state_matrix_tmp.append(state_matrix[j][i])
    print("\n")
    state_matrix = state_matrix_tmp
    print("ciphertext: ")
    print_byte_array(state_matrix)
